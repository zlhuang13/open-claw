    ...
    def do_GET(self):
        ...
        if path in STATIC_MAP:
            ...
            if '..' in rel or not fpath.is_file():
                self.send_error(404)
                return
            ct = {'jpg': 'image/jpeg', 'jpeg': 'image/jpeg', 'png': 'image/png', 'gif': 'image/gif', 'webp': 'image/webp'}.get(fpath.suffix.lower().lstrip('.'), 'application/octet-stream')
            with open(fpath, 'rb') as f:
                data = f.read()
        elif path.startswith('/cat-photos/'):
            rel = path[len('/cat-photos/'):]
            fpath = CAT_PHOTOS_DIR / rel
            if '..' in rel or not fpath.is_file():
                self.send_error(404)
                return
            ct = {'jpg': 'image/jpeg', 'jpeg': 'image/jpeg', 'png': 'image/png', 'gif': 'image/gif', 'webp': 'image/webp'}.get(fpath.suffix.lower().lstrip('.'), 'application/octet-stream')
